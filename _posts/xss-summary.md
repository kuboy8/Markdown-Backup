...
- DOM
---
&#8195;&#8195;首先强力推荐一个系列：**[腾讯实例教程] 那些年我们一起学XSS**，二哥当真无愧XSS第一人。原文贴不上了大家搜镜像吧。
&#8195;&#8195;新手入门都知道XSS有三种：反射型、存储型和DOM型；所需知识JS、Html和CSS；绕过方法主要有字符编码、字符拼接、DOM构造和特殊符号处理异常导致绕过等。我觉得一次成功的XSS主要有几点：
- js、html、css语言的掌握程度
- 对编码的理解
- 对浏览器解码的认识
- 经验
开始都比较容易忽视编码、解码和DOM，因为比较麻烦。一般也都以弹窗来衡量XSS是否成功，但是弹窗后能成功调用外部JS吗？
<!-- more -->
